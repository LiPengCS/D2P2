...
class AutoPrepSGD(object):
    def __init__(self, prep_pipeline, model, loss_fn, model_optimizer, prep_pipeline_optimizer,
        ...
        self.first_order_only = self.params["first_order_only"]
        self.writer = writer
    ...
    def fit(self, X_train, y_train, X_val, y_val, X_test=None, y_test=None):
        ...
        while e < self.params["num_epochs"]:
            ...
            epoch_time = int((time.time() - tic) * 100)

            if e % 100 == 0:
                print("Epoch {}: tr loss:{:.3f} | val acc: {:.3f}| test acc: {:.3f}. Next 100 epoch in {} min {} sec.".format(e, tr_loss, val_acc, test_acc, epoch_time // 60, epoch_time % 60))
    ...
    def train(self, X_train, y_train, X_val, y_val):
        ...
        X_val_batch, y_val_batch = take_random_batch(X_val, y_val, self.params["batch_size"])
