...
def sinkhorn(X, eps=1e-6, max_iter=500):
    ...
    for i in range(max_iter):
        col_sum = X.sum(axis=0, keepdim=True)
        X = X / col_sum
        row_sum = X.sum(axis=1, keepdim=True)
        X = X / row_sum
...
class Transformer(nn.Module):
    ...
    def forward(self, X, is_fit, X_type, max_only=False):
        # train tfs
        X_trans = []
        for tf in self.tf_options:
            if is_fit:
...
class AutoPrepPipeline(nn.Module):
    ...
    def init_parameters(self, X_train, X_val, X_test):
        ...
        # parameters
        self.alpha = torch.nn.Parameter(torch.rand(self.n_tf_types-1, self.n_tf_types-1), requires_grad=True)
    ...
    def forward(self, X, is_fit, X_type, resample=False, max_only=False):
        X_output = deepcopy(X)
        ...
        for i in range(self.n_tf_types - 1):
            ...
            for j in range(self.n_tf_types - 1):
                ...
                # forward
                X_output_i += self.alpha_probs[i, j] * transformer(X_output, is_fit, X_type, max_only=max_only)
                cur_idx += 1

                # if self.alpha_probs[i, j] == 1 and transformer.name == "normalization":
                #     print(X_output.max(), X_output_i.max())
        ...
        # for i in [1, 5, 9]:
        #     X_output_i = torch.zeros_like(X_output)
        #     transformer = self.pipeline[i]
        #     if resample or not transformer.is_sampled:
        #         transformer.sample(temperature=self.temperature, use_sample=self.use_sample)

        #     X_output_i += 1 * transformer(X_output, is_fit, X_type, max_only=max_only)
        #     cur_idx += 1
        #     X_output = X_output_i
    ...
    def transform(self, X, X_type, max_only=False, resample=False):
        "max_only: only do transformer with prob > 0.5 and tf with maximum prob"

        if not self.is_fitted:
            raise Exception("transformer is not fitted")
        return self.forward(X, is_fit=False, X_type=X_type, resample=resample, max_only=max_only)
    ...
    # class AutoPrepPipelineOrder(nn.Module):
    #     def __init__(self, prep_space, temperature=0.1, use_sample=False, diff_method="num_diff", init_method="default"):