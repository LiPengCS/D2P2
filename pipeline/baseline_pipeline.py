...
class Transformer(object):
    ...
    def __init__(self, name, tf_options, in_features, init_tf=None):
        """If init tf options is None, do random initialization"""
        self.name = name
        ...
        if init_tf is None:
            ...
            tf_methods = [tf.method for tf in self.tf_options]
            default_idx = tf_methods.index(init_tf.method)
            tf_idx_choices = [default_idx for _ in range(in_features)]
...
class BaselineFirstTransformer(object):
    ...
    def get_feature_names(self):
        ...
        if self.contain_cat:
            cat_feature_names = [c for c in self.one_hot_encoder.get_feature_names(self.cat_columns)]
        else:
            cat_feature_names = []
    ...
    def get_feature_enc_map(self, num_cat_features):
        ...
        feature_names = self.one_hot_encoder.get_feature_names([str(i) for i in range(num_cat_features)])
        idx_map = {}
...
class BaselinePipeline(object):
    ...
    def __init__(self, method, prep_space, random_state=1):
        assert(method in ["default", "equinn_norm", "random"])
        self.method = method
    ...
    def fit_transform(self, X_train, X_val, X_test):
        ...
        if self.contain_mv:
            if self.method in ["default", "equinn_norm"]:
                init_num_tf = first_tf_dict["default"][0]
                init_cat_tf = first_tf_dict["default"][1]
            else:
                init_num_tf = None
                init_cat_tf = None

            first_transformer = BaselineFirstTransformer(first_tf_dict["num_tf_options"],
            ...
            first_transformer = BaselineFirstTransformer([NumMVIdentity()],
                                                         [CatMVIdentity()],
                                                         in_num_features,
                                                         in_cat_features)
        ...
        for tf_dict in self.prep_space[1:]:
            ...
            elif self.method == "equinn_norm":
                if tf_dict["name"] == "normalization":
                    init_tf = tf_dict["default"]
                else:
                    identity = [tf for tf in tf_dict["tf_options"] if tf.method == "identity"][0]
                    init_tf = identity
            else:
                init_tf = None

            transformer = Transformer(tf_dict['name'], tf_dict['tf_options'], X_trans.shape[1], init_tf=init_tf)
