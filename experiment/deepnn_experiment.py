...
class TransferExperiment(object):
    def __init__(self, data_dir, dataset, prep_space, transfer_prep_path):
...
def run_deepnn(data_dir, dataset, result_dir, auto_prep_space, transfer_prep_path, baseline_prep_space, params, num_random=10):
    # print(dataset, "deepnn", "default")
    # result_dir_default = makedir([result_dir, "default"], remove_old_dir=True)
    # tic = time.time()
    # baseline_default = BaselineExperiment(data_dir, dataset, baseline_prep_space, "default", "five")
    ...
    #     baseline_random = BaselineExperiment(data_dir, dataset, baseline_prep_space, "random", "five", tf_seed=seed)
    ...
    # baseline_random = BaselineExperiment(data_dir, dataset, baseline_prep_space, "random", "five", tf_seed=best_seed)
    ...
    print(dataset, "deepnn", "transfer")
    result_dir_transfer = makedir([result_dir, "transfer"], remove_old_dir=True)
    tic = time.time()
    transfer = TransferExperiment(data_dir, dataset, auto_prep_space, transfer_prep_path)
